...
val LIBRARY_ENTRIES: List<LibraryEntry> = listOf(
    ...
    LibraryEntry(
        ...
        howItWorks = "Every wing's wedge fill is a magnitude of signal, never a raw value — a bigger wedge always means a stronger reading, on every wing, so the visual language stays the same regardless of which wing is active; colour is the separate channel that carries direction (except Volatility — see below). Overall is the Continuation Score (see \"Overall\" above). Momentum is the pair's own D1-only momentum oscillator (0–100, 50 = neutral) — deliberately just the D1 reading, not the D1/H4/H1-blended CMP, for a fast-moving single-timeframe read; because it's bipolar (0 and 100 are opposite extremes, not \"none\" and \"max\"), the wheel folds it around the neutral point (|momentum − 50|) before filling, then takes the square root of that fold so a real-but-modest 8-point deviation shows as a legible ~28% fill instead of a barely-visible 8%. Trend (see \"ADX\" under Momentum & Price Action) is already a pure magnitude — no folding needed, scales directly, and colours from the pair's own direction same as Overall. Volatility (see \"ATR Percentile\" under Setup Quality) is different again: it isn't bullish or bearish at all, so instead of a direction colour it uses the same 20–70 \"sane band\" the ATR Percentile entry already describes — inside the band reads calm/tradeable, outside it (too quiet OR too extended, either one) reads as a caution.",
        whyItMatters = "Simplification Rework, 2026-09-05 (Pieter's own call) — replaced the previous four wings (Potential/Momentum/ADX/Reset Score), which had grown into a toggle whose names didn't map to a mental model anyone could hold onto. Structure was deliberately left off the wheel entirely — it's event-based (a BOS or CHoCH either just happened or it didn't), not a magnitude like the other three, so stretching it into a wedge fill would have meant something different from every other wing. Structure still gets its own Overview row on the pair sheet and its own Structure alert — just not a wheel wing.",
    ),
    LibraryEntry(
        id = "mom1212",
        term = "MOM1212",
