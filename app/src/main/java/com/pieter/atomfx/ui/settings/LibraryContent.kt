...
val LIBRARY_ENTRIES: List<LibraryEntry> = listOf(
    ...
    LibraryEntry(
        ...
        howItWorks = "Both are the same frozen Wilder-RSI and EMA-MACD the technical score already computes for every pair at every timeframe — this card only keeps a short run of past values instead of throwing them away after scoring, so they can be drawn as a line/histogram rather than read as one number. RSI: 0–100, with a dashed reference-line pair and a 50 centreline. The overbought/oversold threshold RSI is actually judged against is still the real, industry-standard 30/70 — the \"Overbought\"/\"Oversold\"/\"Neutral\" state, right-aligned at the bottom of the card, reads the true value. The two dashed lines themselves, though, are drawn at the same on-screen height as %B's own 10/90 lines (Pieter's ask, 2026-09-18 — panel-wide visual consistency, one shared reference band across every glance-panel chart, deliberately decoupled from each indicator's own literal threshold) — so the line can visibly cross into \"Overbought\" before it reaches the (relocated) dashed line; the footer's word is the accurate read, not the line's position against the dashes. No signal line, since plain RSI doesn't have one. Its own line is plain white now too (was tinted at overbought/oversold; the footer state already carries that read) — this is RSI and %B's shared question, \"is this stretched or normal right now,\" just measured by momentum instead of position-in-band. MACD: the histogram (MACD line minus signal line) as bars, tinted by sign, with the MACD and signal lines themselves overlaid — the histogram gets its own vertical scale, independent of the MACD/signal lines, since a histogram (a difference of two similarly-sized series) is mathematically much smaller than either line on its own and would otherwise read flat.\n" +
