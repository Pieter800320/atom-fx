...
val LIBRARY_ENTRIES: List<LibraryEntry> = listOf(
    ...
    LibraryEntry(
        ...
        howItWorks = "(close − lower band) ÷ (upper band − lower band) × 100, on standard 20-period ±2σ bands — corrected 2026-09-18, this line used to say 12-period, left over from before the 20-period rework and never fixed. A 20-period average of %B itself is computed alongside it as a signal line, though the glance-panel chart itself no longer draws it (2026-09-18, Pieter's ask, a cleaner single-line read) — its own reading is still shown as plain text at the bottom of the card instead (2026-09-18, 3rd), so the number the line used to carry is still one glance away. Currency %B's own chart still draws its own signal line, on 12-period bands.\n" +
    ...
    LibraryEntry(
        ...
        howItWorks = "(upper band − lower band) ÷ middle band × 100, on the same standard 20-period bands %B uses, drawn as a line over time. Rising means the bands are expanding — volatility building; falling means they are converging — the market going quiet.\n" +
            ...
            "Squeeze marks — amber dots and a shaded column — flag bars where BandWidth is at its lowest in the past 125 bars. That is Bollinger's own published definition of a squeeze, not a level tuned here. A \"Squeeze (125-bar low)\" legend sits at the bottom of the card, with \"In squeeze\" next to it whenever the current bar qualifies.",
