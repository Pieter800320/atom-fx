...
val LIBRARY_ENTRIES: List<LibraryEntry> = listOf(
    LibraryEntry(
        ...
        howItWorks = "This is `cont`, the frozen Continuation Score (see \"Continuation Score\" under Setup Quality for the full weighted breakdown — timeframe alignment, entry position, CSM divergence, regime fit, session fit). Nothing new is computed for the wheel; the Setup wing (2026-09-09 — renamed from \"Overall\", same wing, same number, name now matches PairSheet's own \"STRONG SETUP\"/\"A+ SETUP\" bands so both surfaces read as one term) is a straight read of the same number that already drives the app's own pair ranking (`rank.py`).",
        whyItMatters = "Simplification Rework, 2026-09-05 (Pieter's own call: \"drop the gate, use a composite score\") — this replaced the old sequential six-factor gate (Regime→Flow→Breadth→Momentum→Structure→Entry, a pair only \"levels up\" 0–6 by clearing each in strict order). That gate is retired app-wide: no more \"blocked at\" factor, no more pass/fail checklist. Setup answers the same question the old Level/Potential number tried to — \"how far along is this idea\" — with one continuous, already-proven number instead of a stepped count. 2026-09-09 bug fix — the wedge's fill (this number) and its colour (bull/bear tint) used to come from two different, sometimes-disagreeing subsystems: the fill was always `cont`, but the tint read `signals.potential[pair].direction`, the OLDER six-factor gate's own direction call, not the direction `cont` itself was computed against — and that field could be entirely absent, forcing grey regardless of the real reading. The tint now reads the SAME D1-else-H4 direction `compute_cont()` used for this exact number, so green/red always agrees with what's actually being scored.",
    ),
    LibraryEntry(
        id = "level-state",
        term = "Setup Bands",
    ...
    LibraryEntry(
        ...
        howItWorks = "Every wing's wedge fill is a magnitude of signal, never a raw value — a bigger wedge always means a stronger reading, on every wing, so the visual language stays the same regardless of which wing is active; colour is the separate channel that carries direction (except Volatility — see below). Every wing is also a FIXED timeframe (2026-09-06, Pieter's own settled call, after trying and rejecting a D1/H4/H1 toggle across the wheel) — Overall is the Continuation Score (see \"Overall\" above, inherently cross-timeframe). Momentum is fixed at H4 (0–100, 50 = neutral, not the D1/H4/H1-blended CMP — switched from D1, 2026-09-09, see \"why it matters\" below). Trend (see \"ADX\" under Momentum & Price Action) is fixed at H4 too — there is no D1 or H1 ADX anywhere in this system, so this isn't a preference, it's the only real number that exists. Volatility (see \"ATR Percentile\" under Setup Quality) is fixed at D1 — it's computed from D1 candles by default (H4 only as a rare fallback when a pair has too little D1 history). Because Momentum's own reading is bipolar (0 and 100 are opposite extremes, not \"none\" and \"max\"), the wheel folds it around the neutral point (|momentum − 50|) before filling, then takes the square root of that fold so a real-but-modest 8-point deviation shows as a legible ~28% fill instead of a barely-visible 8%. Trend colours from the H4 pill specifically (not the pair's overall D1-derived direction) — ADX is an H4 number, and a pair can show real H4 trend strength while the D1 composite reads neutral; tinting off the wrong one drew a contradiction (Pieter's own catch, 2026-09-06). Volatility isn't bullish or bearish at all, so instead of a direction colour it uses its own separate 2-colour system — the same 20–70 \"sane band\" the ATR Percentile entry already describes reads blue inside the band, amber outside it either way. 2026-09-10 (Pieter's ask, settled after two iterations) — the wheel's colour system is now: green/grey/red, a genuine 3-state traffic light, for Overall/Trend/Momentum (bull/neutral/bear); blue/amber, a separate 2-colour system sharing no hue with the traffic light, for Volatility alone (sane/caution). Amber is reserved exclusively for Volatility now — nothing on Overall/Trend/Momentum uses it any more. Two things changed to land the traffic light: Trend's old \"ADX 25+ but H4 pill neutral\" mismatch used to get its own amber flag instead of grey — it now folds into plain neutral (grey) like any other flat Trend reading, no separate callout, matched exactly on the pair sheet's own Overview Trend row too. And Momentum gained a real neutral dead zone it never had before (was a hard >=50 split, always green or red, nothing in between) — now matches CMP's own exact thresholds (`>=60` bull, `<=40` bear, 41–59 neutral), since CMP is the closest existing precedent for the same 0–100/50-neutral shape. (First cut of this change made the wheel's whole \"neutral\" token blue instead of grey, matching Volatility's own blue — Pieter didn't like blue on the directional three, so it split into two separate tokens: grey neutral for the traffic light, its own distinct blue for Volatility's sane band, `wheelNeutral`/`wheelSane` in `Color.kt`.)",
        whyItMatters = "Simplification Rework, 2026-09-05 (Pieter's own call) — replaced the previous four wings (Potential/Momentum/ADX/Reset Score), which had grown into a toggle whose names didn't map to a mental model anyone could hold onto. Structure was deliberately left off the wheel entirely — it's event-based (a BOS or CHoCH either just happened or it didn't), not a magnitude like the other three, so stretching it into a wedge fill would have meant something different from every other wing. Structure still gets its own Overview row on the pair sheet and its own Structure alert — just not a wheel wing. 2026-09-06 — the fixed timeframes turn out to form a genuine consensus funnel, not an arbitrary mix: the Regime hub sets the macro backdrop, H4 Trend confirms something persistent is actually happening at that same structural level (and directly shares its H4 timeframe with the H4 inputs — CSM divergence, regime fit, the ADX gate — that dominate Overall's own formula), Momentum is the actual tradeable direction. 2026-09-09 (several changes, Pieter's own asks) — the hub moved from H4 Regime to D1 Regime (\"the H4 Regime changes too much\"), making Regime the wheel's one deliberately slower \"master direction\" timeframe; and Momentum moved from D1 to H4, since D1 Momentum was re-reading roughly the same candles D1 Regime already votes on (same timeframe, two lenses, not real multi-timeframe confluence) — H4 Momentum genuinely checks whether a faster timeframe still supports the D1 bias, a real top-down read (Elder's \"triple screen\" framing: a slower timeframe sets the only allowable bias, a faster one confirms against it). So three of the four wings — Regime, Trend, Momentum — are a genuine confluence funnel answering \"which way, and does it hold up.\" Volatility is deliberately the odd one out, not a fourth vote on direction at all: after reviewing D1/H4/H1 ATR side by side, it stayed D1 (see the ATR Percentile entry for why a faster timeframe wouldn't have helped — ATR lags at any timeframe, it just cycles faster). Its job is a risk-sizing question — \"is it sane to size an entry right now\" — not a directional one, which is exactly why it's the one wing that was never bull/bear-tinted.",
    ),
    LibraryEntry(
        id = "mom1212",
        term = "MOM1212",
    ...
    LibraryEntry(
        ...
        howItWorks = "Every one of 18 fixed currency pairs (2026-09-10 — was 16; EUR/JPY and GBP/JPY added, closing a gap where two fully-tracked pairs never fed currency strength for EUR/GBP/JPY at all) contributes an ATR-normalised return to both its base currency (positive) and its quote currency (negative). Average each currency's contributions, then stretch the 8 currencies' averages so the weakest sits at 0 and the strongest at 100. D1 blends 14-bar D1 (70%) and H4 (30%) returns; H4 blends 5-bar H4 (80%) and 8-bar H1 (20%); H1 is pure 6-bar H1.",
    ...
    LibraryEntry(
        ...
        howItWorks = "Each currency appears in a different number of the 18 CSM pairs (USD in 7, AUD in 5, JPY in 6, GBP in 5, EUR in 4, CHF/CAD/NZD in 3 — 2026-09-10, was 16 pairs before EUR/JPY and GBP/JPY were added, see the CSM entry above). Breadth is how many of those relationships point the same way as the currency's own net direction, divided by how many it appears in at all — always compared as a percentage, never a raw count, because the totals differ per currency. 70% or above is \"strong,\" 50% or above is \"moderate,\" below that is \"weak.\"",
    ...
    LibraryEntry(
        ...
        whyItMatters = "Too low, below roughly 20, means the market's too quiet to trust a breakout; too high means you might be entering into a volatility spike. 20–70 is the app's own \"sane\" entry band. Crossing 90 fires a Volatility alert (State-Transition Alerts) — tap its book icon (2026-09-04) for the Volatility Playbook, on why an expansion is a risk-sizing signal first, an opportunity signal second. 2026-09-05 — this is also the wheel's Volatility wing and the pair sheet's Overview VOLATILITY row (same field, same 20–70 band, just surfaced in two more places). 2026-09-09 (Pieter's own chart review, D1/H4/H1 side by side) — deliberately stays D1, not moved to H4 or H1: ATR is a lagging average of already-printed bar ranges, true at any timeframe — a healthy, grinding trend can show falling ATR the entire way (small consistent bars, steady direction), reading identically to a dead consolidation, because ATR measures bar range, not directional persistence. A faster timeframe doesn't fix that, it just cycles the same lag more often. So this is read as a risk-sizing gate (\"is it sane to size an entry right now\") — the same job pros actually use ATR for (stop distance, position sizing), not as a fourth vote on direction alongside Regime/Trend/Momentum. \"Grey/neutral\" on the wheel's Volatility wing means conditions are sane for whatever the other three wings are already suggesting, not a buy signal on its own.",
    ),
    LibraryEntry(
        id = "h4-regime",
        ...
        howItWorks = "Four votes are cast: (1) safe-havens JPY/CHF vs risk currencies AUD/NZD/CAD by CSM, (2) USD vs the rest of the majors (EUR/GBP/AUD/NZD/CAD) by CSM, (3) pill direction across the same commodity-bloc pairs as vote 1 (AUDUSD, NZDUSD, AUDJPY, NZDJPY, CADJPY, USDCAD — inverted, since CAD is the quote there), (4) a ranging override that forces \"Ranging\" outright if fewer than 40% of all 12 pairs have any directional pill at all. Two or more votes agreeing on the same side wins; all three agreeing is High confidence, two is Medium. No majority is Mixed. Votes 1 and 2 are both CSM point-gap checks; CSM's 0–100 scale is min-max rescaled every single scan (whichever currency is strongest THIS scan is forced to exactly 100), so a gap can look identical on a dead-quiet day and a violently trending one. To catch that, each timeframe's raw pre-normalisation spread is tracked over its trailing ~60 scans (2026-09-06); when today's spread ranks in the bottom 20% of that history — thin enough that the gap is more likely rescaling noise than a real signal — votes 1 and 2 are both forced to Mixed rather than trusted. 2026-09-10 (Pieter's sign-off, FX-methodology pass) — vote 3 used to mix the commodity bloc (AUD/NZD) with GBP/EUR, which aren't classic risk currencies (their moves are driven far more by ECB/BoE policy and regional growth/political risk than global risk appetite); vote 3 now measures the exact same AUD+NZD+CAD-vs-havens concept as vote 1, just confirmed independently via pills instead of CSM, and CAD (previously absent from every risk-currency vote in this classifier) is now represented in votes 2 and 3 both. The exact same classifier also runs at H4 and H1 (all three shown side by side on the Regime sheet) — only which timeframe's votes are the wheel's own \"headline\" read differs (D1, since 2026-09-09; was H4 — \"the H4 Regime changes too much\").",
        whyItMatters = "This is the regime shown at the wheel's hub, and it gates Factor 1 — nothing can pass Regime under a Mixed or Ranging backdrop. Tap the book icon on the Regime sheet (2026-09-04) to open its Regime Playbook — the deeper per-regime read of what the three votes and the score actually mean for whichever regime is live, and how this technical read relates to the separate Macro Archetype and to Gold Signal's own confirmation gate (which still specifically requires H4 AND H1 agreement, independent of which timeframe the hub itself shows).",
    ),
    LibraryEntry(
        id = "macro-archetype",
