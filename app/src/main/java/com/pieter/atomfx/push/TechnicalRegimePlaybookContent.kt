 ...
 * `regime_flip`: the structural regime shown at the wheel's hub.
 *
 * 2026-09-09 (Pieter's ask) — the hub (and this playbook's trigger) switched from the H4 instance
 * to the D1 instance of `classify_regime` ("the H4 Regime changes too much"). The algorithm below
 * is identical either way (frozen, tf is just a parameter) — only which timeframe's pills/CSM feed
 * the votes changed, so every "at H4" reference to the vote inputs below became "at D1". Gold
 * Signal's own separate H4-AND-H1 confirmation requirement (scan_news.py, untouched) still reads
 * H4 and H1 specifically — that's a different, frozen consumer of this same classifier, not this
 * hub, and stays as-is.
 *
 * 2026-09-10 (Pieter's sign-off, FX-methodology pass) — Vote 3 used to mix the commodity bloc
 * (AUD/NZD) with GBP/EUR, which aren't classic risk currencies (their moves are driven far more
 * by ECB/BoE policy and regional growth/political risk than global risk appetite — Pieter's own
 * catch). Vote 3 now measures the exact same AUD+NZD+CAD-vs-havens concept as Vote 1, just via
 * pills instead of CSM — a genuine independent-methodology confirmation of the same idea, not a
 * third, differently-defined opinion. CAD, previously absent from every risk-currency vote in
 * this classifier, is now represented in Vote 2 and Vote 3 both.
 *
...
val TECHNICAL_REGIME_PLAYBOOK: Map<String, TechnicalRegimeEntry> = listOf(
    TechnicalRegimeEntry(
        regime = "Risk-On",
        coreStory = "Two or more of three price-based checks agree that risk appetite is genuinely broad-based right now: safe-havens (JPY/CHF) are being sold relative to commodity/risk currencies (AUD/NZD/CAD) on CSM, the dollar is being sold relative to the rest of the majors on CSM, or a clear majority of six commodity-bloc pairs (AUDUSD, NZDUSD, AUDJPY, NZDJPY, CADJPY, USDCAD — the last one inverted, since CAD is the quote there) have bullish pills at D1. This is a purely technical read — it doesn't know or care WHY risk appetite is up, only that price itself is confirming it from more than one angle at once.",
        votesExplanation = "Vote 1 (safe-haven CSM): JPY+CHF average vs AUD+NZD+CAD average — needs a 15-point CSM gap the risk side's way to count as risk-on. Vote 2 (USD CSM): USD vs EUR/GBP/AUD/NZD/CAD average — needs a 20-point gap the non-USD side's way. Vote 3 (risk-basket pills): needs at least 2 more bullish than bearish pills across the same AUD/NZD/CAD-vs-havens basket as Vote 1, read via pills at D1 instead of CSM. Two of these three agreeing is enough to call the regime; the third can be dissenting or flat.",
        ...
        falsificationChecklist = listOf(
            ...
            "Does the Macro Archetype agree? If the cross-asset picture (Macro tab) is showing something closer to Regime B (US rate dominance) or Regime F (Inflation repricing) at the same time, this technical Risk-On read may be a shorter-lived price move, not a durable backdrop.",
    ...
    TechnicalRegimeEntry(
        regime = "Risk-Off",
        coreStory = "The mirror image of Risk-On: two or more of the same three checks now agree the other way — safe-havens are being bought relative to commodity currencies, USD is being bought relative to the rest of the majors, or a clear majority of the same six commodity-bloc pairs have bearish pills at D1. Same caveat as Risk-On: this is price/CSM only, it doesn't distinguish a genuine growth scare from a liquidity event from a simple rate-driven USD bid — for that distinction, this is exactly what the Macro Archetype's Regime D/E split exists for.",
        votesExplanation = "Same three votes as Risk-On, run the other direction: safe-haven CSM 15+ points above the risk-currency average, USD CSM 20+ points above the EUR/GBP/AUD/NZD/CAD average, or 2+ more bearish than bullish pills across the same basket at D1.",
        ...
        falsificationChecklist = listOf(
            ...
            "Does this coincide with a VIX spike large enough to suggest Regime E (Liquidity stress) on the Macro tab, rather than an ordinary Regime D read? The technical regime alone can't tell you which — it only says \"risk-off\", not \"how violent\".",
    ...
    TechnicalRegimeEntry(
        regime = "Ranging",
        coreStory = "This is not a vote outcome at all — it's an override that runs BEFORE the three votes are even tallied. If fewer than 40% of all 12 pairs (not just the six risk pairs) have any directional pill at D1, the classifier forces \"Ranging\" outright and never gets to ask what the safe-haven/USD/risk-basket votes would have said. Ranging and Mixed are easy to treat as synonyms for \"unclear\" but they're structurally different diagnoses: Mixed means the votes disagreed; Ranging means there wasn't enough directional data across the whole pair set to even run the vote.",
        votesExplanation = "A single check, run first: directional pill count across all 12 pairs at D1, divided by 12. Below 40% forces Ranging and skips the vote tally entirely — the three votes described in the Risk-On/Risk-Off entries never execute this scan.",
        ...
        relationToOtherSystems = "Independent of the Macro Archetype. A broad lack of directional conviction in price doesn't mean the cross-asset/macro backdrop is quiet too — the Macro tab can still show a confident archetype call while most individual pairs sit range-bound. Gold Signal never confirms on Ranging.",
        falsificationChecklist = listOf(
