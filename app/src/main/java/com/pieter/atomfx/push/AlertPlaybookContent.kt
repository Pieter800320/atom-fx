...
val CONVICTION_EXTREME_PLAYBOOK: Map<String, AlertPlaybookEntry> = listOf(
    AlertPlaybookEntry(
        ...
        relationToOtherSystems = "This is the live version of the exact check the Regime Playbook's own Regime E (Liquidity stress) and Regime J (Carry unwind) entries point back to — both explicitly name Conviction data as the tool for spotting the positioning extreme that a purely price-based regime read can't see on its own. If a Conviction Extreme alert and a Regime E/J read show up close together, that's the two systems corroborating the same underlying risk from different data.",
        falsificationChecklist = listOf(
...
val GOLD_SIGNAL_PLAYBOOK: Map<String, AlertPlaybookEntry> = listOf(
    ...
    AlertPlaybookEntry(
        ...
        relationToOtherSystems = "Same Technical Regime gate as the bullish case. Worth cross-checking against WHICH kind of risk-off is live on the Macro tab (Regime D: Growth-scare risk-off, E: Liquidity stress, and I: Energy-cost dollar bid can all read \"Risk-Off\" on the Technical Regime first) — gold's own behaviour can differ meaningfully across those, particularly Regime E where gold itself can be sold for pure liquidity/margin reasons even though it's nominally the \"safe-haven\" asset.",
        falsificationChecklist = listOf(
