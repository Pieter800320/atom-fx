...
| 9 | Inside-bar breakouts — Pieter's idea | A close beyond the mother bar's range within 3 bars of an inside bar continues more often (1 x ATR race, 20 bars) than an ordinary prior-bar-range breakout, on D1 (native 2009-2026) and H4 (NY) | ACTIVE 2026-09-20 (pre-registered) | - | - | "Experiment 9" below; `tools/backtest_ideas.py --exp inside` |
| 10 | COT extremes held for weeks (contrarian) — Pieter's idea | Combined Legacy-COT percentile >= 90 (crowded long, expects pair DOWN) / <= 10 (expects UP), held 8 weeks, beats the pair's own drift | ACTIVE 2026-09-20 (pre-registered) | - | - | "Experiment 10" below |
| 11 | Monthly per-pair carry — Pieter's idea | Long the higher-yielding currency (BIS policy-rate difference >= 0.5 pp), rebalanced monthly, earns a positive total return (spot + carry accrual) | ACTIVE 2026-09-20 (pre-registered) | - | - | "Experiment 11" below |
