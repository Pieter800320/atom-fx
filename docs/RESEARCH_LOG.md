...
| 8 | D1 and H4 Crowd scores coincide (both > 30, same side) — Pieter's idea | When the H4 score AND the last completed D1 score are both above 30 on the same side, the H4 bar reverses more often than like-for-like baseline bars (and more than H4-alone flags) | RUN 2026-09-19 (once, no changes after pre-registration) | T=30: 318 events, reversed 53.1% vs 48.7% baseline, lift +4.4% (CI -1.8..+10.5); vs D1-state-matched baseline +2.3%; H4-alone -0.9%. T=60: 9 events | **FAIL** (F2, CI). Closest result so far but not significant; T=60 INCONCLUSIVE | see "Experiment 8" below; `tools/backtest_crowded_reversal_confluence.py` |
| 9 | Inside-bar breakouts — Pieter's idea | A close beyond the mother bar's range within 3 bars of an inside bar continues more often (1 x ATR race, 20 bars) than an ordinary prior-bar-range breakout, on D1 (native 2009-2026) and H4 (NY) | RUN 2026-09-20 (once) | P1 D1 native 2009-26: 3,917 events, continuation 51.3% vs 49.7% ordinary breakouts, lift +1.7% (CI +0.2..+3.2). P2 H4 NY: 8,639 events, +0.5% (CI -0.5..+1.5) | **NOT SUPPORTED** (fails the 5-point size gate; D1 tendency real but tiny) | "Experiment 9" below; `tools/backtest_ideas.py --exp inside` |
| 10 | COT extremes held for weeks (contrarian) — Pieter's idea | Combined Legacy-COT percentile >= 90 (crowded long, expects pair DOWN) / <= 10 (expects UP), held 8 weeks, beats the pair's own drift | RUN 2026-09-20 (once) | h=8w: 179 events, lift -0.04 sigma (CI -0.24..+0.17), hit 44.1% vs 49.5%; h=4w +0.05; h=12w +0.01 | **NOT SUPPORTED** (no contrarian effect at any horizon) | "Experiment 10" below |
| 11 | Monthly per-pair carry — Pieter's idea | Long the higher-yielding currency (BIS policy-rate difference >= 0.5 pp), rebalanced monthly, earns a positive total return (spot + carry accrual) | RUN 2026-09-20 (once) | 227 months, total +1.55%/yr (CI -2.48..+5.39), Sharpe 0.17; carry accrual +2.05% offset by spot -0.50%; max drawdown -28.6% | **FAIL** (CI, Sharpe) | "Experiment 11" below |
...
### Result (run once on 2026-09-19; pre-registration commit `ac18ee0`; outputs `data/backtest/crowded_reversal_exp8_t30_2026-09/`, `..._t60_...`)
Nothing changed between the pre-registration commit and these runs; the one-pair smoke (n=21, no conclusion drawn) altered no parameter or gate.

| T = 30, H4 NY-aligned, corrected bars | events | reversal rate | baseline | lift (95% CI) |
|---|---|---|---|---|
| **`coincide` (H4 and last completed D1 both > 30, same side)** | **318** | **53.1%** | 48.7% (all H4 bars) | **+4.4% (-1.8 .. +10.5)** |
| `coincide_dmatched` (baseline = H4 bars whose D1 state is > 30) | 318 | 53.1% | 50.8% | +2.3% (-3.8 .. +8.2) |
| `h4_alone` (H4 > 30, D1 state NOT > 30) | 2,156 | 48.1% | 49.0% | -0.9% (-4.1 .. +2.4) |

Gates: PASS F1, F3 (halves +2.7% / +6.6%), F4 (ex-JPY +2.8%), F5 (ex-GBPJPY +3.2%), F6 (53.1% > 50%), F7 (beats `h4_alone`); **FAIL F2 (lift 4.4 < 5 points) and the CI lower bound (-1.8 < 0).** T = 60: 9 events, 66.7% vs 49.5%, lift +17.1% (CI -8.1 .. +49.9): INCONCLUSIVE (F1), as pre-registered.

**Verdict (pre-registered): FAIL.** In plain terms: when the D1 and H4 scores are both above 30 on the same side, the H4 bar reversed 53.1% of the time against 48.7% for an average H4 bar — the highest raw rate of any test in this series, positive in both
halves and in the ex-JPY and ex-best-pair cuts, and clearly better than H4 alone (48.1%). But the interval includes zero, the lift is below the 5-point bar, and against a baseline matched on the D1 state it shrinks to +2.3% (CI -3.8 .. +8.2): most of the raw
difference is the D1 state itself, not the H4 confirmation. **Size:** +4.4 points on a 1:1 race is about +0.06 R per trade before costs; 318 events over 5.7 years across 12 pairs is roughly 56 a year, so about +3 R a year — a rounding error. **Caveats:** ninth question on
one data family (a nominal near-miss is exactly what repeated testing produces by chance); the outcome is a reversal race, not a trade; costs, entries and exits were not modelled. The only clean confirmation is FORWARD (live) data; the app already records the scores every scan.

## Experiments 9, 10, 11 — inside bars, COT holds, carry (pre-registration, 2026-09-20)
**Written and committed BEFORE any full-run outcome. Disclosure first:** a plumbing smoke on EURUSD + AUDUSD only (200 bootstraps) was run BEFORE this entry to catch bugs and it printed outcomes: inside bars n=642, lift +2.2% (CI -0.8..+5.7);
COT 8-week n=27, standardised lift -0.54 sigma (hit rate 26% vs 50%; i.e. continuation, not reversal, on those two pairs); carry 200 months, +0.50% a year. **No parameter, gate or definition below was changed after seeing them;** two pairs and n=27 are not evidence.
Common: 12 pairs, month-clustered bootstrap 5000, seed 20260919, no costs modelled, a PASS is a hypothesis for a FORWARD test only (twelfth family of questions on this data; multiplicity applies).

### Experiment 9 — inside bars
- **Inside bar:** high < previous high AND low > previous low (strict); only the FIRST inside bar of a sequence counts; the **mother bar** is the bar before it.
- **Event:** the first bar within 3 bars after the inside bar whose CLOSE is above the mother's high (up-break) or below the mother's low (down-break). Race (as Experiments 2-8: 1 x ATR(14), 20 bars, same-bar-both -> continuation, timeout -> not) from that bar's close; success = the break DIRECTION continues.
- **Baseline (the control):** ordinary prior-bar-range breakouts, i.e. every bar closing above the previous bar's high (for up events) / below the previous bar's low (down events), same pair, side, month. Lift = do inside-bar breakouts continue more than ordinary breakouts?
- **Primaries, both required:** P1 D1 native daily 2009-2026 (`d1utc`); P2 H4 on the app's NY-session blocks (`h4cny`, corrected bars). Informational: D1 NY-close (`d1c`); `inside_vs_all` (baseline = all bars).
- **Gates (per primary):** F1 >= 100 de-clustered events (else INCONCLUSIVE); F2 lift >= 5 points AND 95% CI lower bound > 0; F3 lift > 0 in both halves; F4 excluding JPY crosses > 0; F5 excluding the best pair > 0; F6 continuation rate > 50%. SUPPORTED only if both pass; one = SUGGESTIVE only; none = NOT SUPPORTED.
- **Not tested:** other windows, mother-bar size filters (NR4/NR7), pin-bar variants, entries at the mother's level, stops, costs.

### Experiment 10 — COT extremes held for weeks (contrarian)
- **Signal:** the port's combined percentile for the pair (Legacy non-commercial net, 156-week percentile, base leg and inverted quote leg averaged), already lagged to the Tuesday report of the PREVIOUS week (no look-ahead); rising edge into >= 90 (top: crowded long, expects the pair DOWN) or <= 10 (bottom: expects UP). Entry at that day's close (the first day of the week the value changes = Monday, conservative).
- **Outcome:** the pair's forward return over h weeks (5h D1 bars) signed by the expected direction, divided by that pair's standard deviation of h-week returns ("standardised"), minus the pair's OWN mean standardised signed return over all eligible days (drift-adjusted baseline); hit = signed return > 0. De-clustered per pair and side: a new event only after the previous one's h weeks. Month-clustered bootstrap over events.
- **Primary:** h = 8 weeks, thresholds 90/10, native D1 2009-2026 (percentile needs 156 weeks, so events start ~2012). Informational: h = 4 and 12. Only the CONTRARIAN direction is hypothesised; a negative lift means the extremes were followed by continuation.
- **Gates:** F1 >= 100 events; F2 mean standardised lift CI lower bound > 0 AND hit-rate lift >= 5 points; F3 both halves > 0; F4 ex-JPY > 0; F5 ex-best-pair > 0; F6 hit rate > 50%.
- **Not tested:** thresholds other than 90/10, momentum (continuation) use, other holding rules, stops, costs.

### Experiment 11 — monthly carry
- **Data:** BIS central-bank policy rates (daily, `WS_CBPOL`, 8 currencies, to Sep 2026) and native D1 month-end closes 2009-2026; no external key needed.
- **Rule:** at each month-end, for each pair: difference d = policy rate(base) - policy rate(quote) in percentage points; if |d| >= 0.5 pp go LONG the pair when d > 0 and SHORT when d < 0 (i.e. always long the higher-yielding currency); else flat. Hold one month.
- **Return:** total = sign x (spot return over the month + d / 1200) (carry accrual approximated as d/12 per month; no forward points, swap markup or costs). Portfolio = equal weight across active pairs each month; statistics on the monthly series.
- **Gates:** F1 >= 100 months; F2 annualised total return with bootstrap 95% CI lower bound > 0; F3 both halves > 0; F4 excluding JPY pairs > 0; F5 excluding the best calendar year > 0; F6 annualised Sharpe >= 0.3. Informational: spot part vs carry part, max drawdown, positive-month share.
- **Not tested:** cross-sectional ranking, volatility filters, leverage/sizing, other thresholds than 0.5 pp, real broker swaps (retail swaps are typically well below the policy-rate difference).
- **Prior (stated so it can be held against me):** carry has a long, well-documented positive average return with crash risk (2008, 2015, 2020) and a weaker record since 2008 on G10; I expect a small positive mean that may not clear the CI gate.

### Results of Experiments 9, 10, 11 (run once on 2026-09-20; pre-registration commit `8a5a634`; outputs `data/backtest/ideas_exp9_*`, `ideas_exp10_*`, `ideas_exp11_*`)
Nothing changed between the pre-registration commit and these runs.

**Experiment 9 — inside bars: NOT SUPPORTED.**
| bar set | events | continuation rate | ordinary-breakout baseline | lift (95% CI) | verdict |
|---|---|---|---|---|---|
| **P1 D1 native 2009-2026** | 3,917 | 51.3% | 49.7% | **+1.7% (+0.2 .. +3.2)** | FAIL (F2: below 5 points) |
| **P2 H4 NY-aligned** | 8,639 | 49.8% | 49.3% | **+0.5% (-0.5 .. +1.5)** | FAIL (F2, CI, halves, F6) |
| D1 NY-close 2021-26 (info) | 1,378 | 49.3% | 48.3% | +1.0% (-1.5 .. +3.8) | FAIL |
Plain reading: an inside-bar breakout continues about as often as an ordinary prior-bar breakout. On D1 there is a statistically detectable but tiny edge (+1.7 points, about +0.03 R per trade before costs); on H4 nothing.

**Experiment 10 — COT extremes held (contrarian): NOT SUPPORTED.** h = 8 weeks: 179 de-clustered events, standardised lift -0.042 sigma (CI -0.238 .. +0.174), hit rate 44.1% vs 49.5% (halves -0.134 / +0.094; top -0.024, bottom -0.066). h = 4 weeks: 206 events, +0.045 sigma, hit 49.5%; h = 12 weeks: 153 events, +0.014 sigma, hit 49.7%.
Plain reading: crowded speculative positioning (extreme 156-week percentiles) was NOT followed by pair reversals over 4-12 weeks; if anything the 8-week outcome leaned toward continuation (hit rate 44%), not significant.

**Experiment 11 — monthly carry: FAIL.** 227 months, average 7.4 active pairs: annualised total return +1.55% (CI -2.48% .. +5.39%), volatility 9.0%, Sharpe 0.17, 55% positive months. Split: carry accrual +2.05% a year, spot -0.50% a year (the higher-yielding currency lost value on average, eating a quarter of the carry);
max drawdown -28.6%. Halves +0.81% / +2.30%; ex-JPY +0.44%; ex-best-year (2009) +0.45%. Fails F2 (CI includes zero) and F6 (Sharpe < 0.3). Plain reading: positive on average but small, inside the noise, with a large drawdown; policy-rate carry across these 12 pairs did not clear the bar in 2009-2026. (Costs and real broker swaps, typically below the policy-rate difference, would make it lower.)

**Overall:** three more ideas, no edge that clears the gates. The only nominal signals are tiny (D1 inside bars +1.7 points; carry +1.6% a year with a CI through zero). Twelfth family of questions on one data set: none of these should be traded on.
