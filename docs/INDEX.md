...
- 2026-09-19 (7th) — **Pine debug export added to `pine/crowded_reversal.pine`** (group 8, OFF by default): 18 Data-Window-only plots of the script's own intermediate values (%B, z, stretch, RSI, ADX, EMA200, COT percentile, divergence, climax, regime, points, raw scores) so a TradingView "Export chart data" CSV can be compared bar by bar with the app's port. Changes no score, flag or alert. Prepared to find why some TradingView flags differ from the app's (BUILD_STATUS: TradingView parity).

- 2026-09-19 (8th) — **FOUND: Twelvedata's hourly forex timestamps are Australia/Sydney local time, not UTC** (+10 h AEST / +11 h AEDT), from a single TradingView screenshot. The real-FX-week fix is DISABLED by a kill switch (`BUILD_STATUS.md` item 18, `scanner/FROZEN.md`). Affects the frozen D1/H4 boundaries, the Crowd score's "NY-close" bars, and the research stores; a timestamps-to-UTC conversion is proposed and awaits Pieter's sign-off.

- 2026-09-19 (9th) — **Timestamp fix shipped: Twelvedata's Sydney-time hourly labels are converted to true UTC (`fx_week_v2`) and the real-FX-week filter is re-enabled.** Validated against six TradingView Data Window readings: 5 of 6 Crowd scores exact, prices within 0.1-3 pips. `BUILD_STATUS.md` item 18 has the numbers and the open items (first production scan to verify; Experiments 2-7 to re-run on converted bars; research-port ADX differs from TradingView's on some bars).
