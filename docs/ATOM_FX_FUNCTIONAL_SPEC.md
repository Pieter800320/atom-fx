...
- ~~**New alert types** (pair advances to level 6; top-pair change).~~ **Shipped 2026-09-04** as the Signals Roadmap's Phase 1 — six edge-triggered alerts (`potential_state`, `structure_event`, `regime_flip`, `archetype_change`, `volatility_spike`, `tf_alignment`), each with its own opt-in Settings toggle, clearly separate from the frozen gold/level alerts.
- ~~**Positioning / COT data**~~ (handbook's "positioning is a multiplier"). **Shipped 2026-09-04** as the Signals Roadmap's Phase 3 — a `conviction` overlay (`-100..+100` per currency) from CFTC's free public TFF report, on its own weekly cadence (`scanner/scan_cot.py`), surfaced on the Currency Detail sheet plus a `conviction_extreme` alert. Not an archetype-engine axis or a pair-sheet asymmetry flag as originally sketched here — a standalone overlay instead; revisit wiring it into the archetype engine as a later refinement if it proves out.
