...
**Rule #1 exception, 2026-09-19 (Pieter's explicit sign-off) — real-FX-week bars.** No frozen calculation file was edited; what changed is the INPUT `scan_h1.py` hands them. Twelvedata has returned market-closed weekend bars since 2026-01-11, and every frozen number (the aggregator's D1/H4, pills, ADX, regime, CSM) was being computed with them in its window. `scan_h1.py` now passes each pair's H1 through `scanner/extend/fx_week.py` — closed-market rows dropped, and for the 12 wheel pairs merged with a persistent H1 history (`data/h1_history/<PAIR>.csv`) so the frozen D1 scorer keeps its >= 210 bars without a second API call (dropping the weekend rows from a 5,000-row fetch alone leaves ~179 D1 bars). The frozen aggregator therefore again receives what it was written against; `aggregator.py`'s own docstring ("Alignment is exact") is unchanged. The first scan under the new convention step-changes the inputs to every edge-triggered alert, so `scan_h1.py` suppresses the state-transition pushes and the Gold Signal PUSH for that one scan (state is still saved; `bars_convention` in signals.json is the marker), and `scan_cot.py` skips its one Conviction comparison. Measured effect and the full record: `scanner/FROZEN.md`, `BUILD_STATUS.md` item 18.
...
percent_b_board    {line:[float,…], signal:[float,…], dates:[str,…]}  — market-wide average %B
                    (2026-09-10): the pointwise mean of every pair's own `bb_d1.pctb`/`.pctb_sma`
                    (see below), across whichever pairs have a bb_d1 read. Rebuilt fresh every
                    scan straight from D1 closes — no scan-to-scan history round-trip needed
...
Similarly, `pairs.<PAIR>.bb_d1` (Signals Roadmap §5, D1 Bollinger touch state) gained new fields
alongside its existing `touching`/`sma`/`upper`/`lower`/`width_pct`/`width_trend` (2026-09-10):
...
1. **Gold signal** — the underlying *qualifying* condition is still exactly `gs_direction != "neutral" AND h4_confirmed AND h1_confirmed AND h4_conf in {Medium, High}`, computed by the frozen gold-signal block — do not touch that computation. **2026-09-17 (Pieter's explicit sign-off, not a silent deviation — see the exact discussion in git history the same day):** the *push itself* is now additionally edge-triggered, same convention every other alert in the app uses (Signals Roadmap §1) — it fires only on the not-qualifying → qualifying transition, or a direction flip while it stays qualifying, rather than re-firing every single hour the condition happens to still hold. Before this, Gold Signal was the one alert in the app that could genuinely spam: a sustained regime could hold it true for many consecutive hours, each one pushing the identical information. The reference app's own always-push behavior is preserved in spirit nowhere else in this codebase (every other alert here is deliberately edge-triggered), so bringing this one alert in line was judged a correctness fix, not a scope change to *what* qualifies — see `scan_h1.py`'s own `_gold_signal_should_push()` docstring for the exact edge-trigger logic.
2. **Price level alerts** — `level_ema_alerts.check_levels(...)` against the latest pair close (the reference passes the most-recent H4 bar close), fires on touch and deactivates the level. (Frozen — untouched.)

`check_ema_touches` remains present but disabled exactly as in the reference (gold signal is the sole proactive alert). Extending which conditions fire is still out of scope for v1 by default — Rule #1 forbids *silently* changing when a signal fires; row 1 above is the one deliberate, explicitly-discussed exception on record, not a precedent for doing this again without the same stop-and-ask.
...
{ "message": {
    ...
    "data": { "type":"gold_signal|level_alert|structure_event|regime_flip|archetype_change|volatility_spike|tf_alignment|conviction_extreme|bb_touch|recommendation|crowd_score",
...
- **State-transition alerts (Signals Roadmap §2, EXTEND-tier).** `scanner/extend/state_alerts.py` adds six edge-triggered types on top of the two frozen ones above — `structure_event`, `regime_flip`, `archetype_change`, `volatility_spike`, `bb_touch`, `tf_alignment` — each firing only on a transition against the previous scan's `signals.json`, never on a level that's merely still true. They ride the same hourly scan and the same `send_push_alert` call, so no new transport or schema shape — just six new `type` values. Matching Settings toggles exist client-side (`NotificationPrefs` — Structure, Regime, Volatility, BB Touch, Alignment) covering the six types; Structure covers both BOS and CHoCH, Regime covers both the H4 flip and an Archetype change. (Corrected 2026-09-17 — this previously listed the retired `potential_state`/"Setup" type, removed outright 2026-09-17 in favor of the richer Recommendation alerts, and omitted `bb_touch`, which had actually shipped 2026-09-09.)
- **`crowd_score` (Signals Roadmap §5c Phase 2, 2026-09-19, EXTEND-tier).** `state_alerts.py::_crowd_score_alerts` fires when a pair's Crowd top or bottom score on the newest COMPLETED D1 or H4 bar sits in a higher band (>0, ≥20, ≥40, ≥60) than in the previous scan's `crowd_series.<tf>.latest` — rising edge only, one push per pair × timeframe × side; a missing previous `latest` (first sighting, the migration scan) never fires. Extra push `data` fields: `timeframe` (`d1`/`h4`), `side` (`top`/`bottom`), `level`, `prev_level`; `direction` is `bear` for top, `bull` for bottom (BB-touch convention). **The minimum level is applied on the device**, not here: the topic is shared, so `AtomFxMessagingService` drops a push whose `level` is below that person's per-timeframe minimum (any/20/40/60, `push/CrowdAlertFilter.kt`). Its own "Crowd score alerts" toggle, **off by default** (the score is context, not a validated signal). Deeplinks to `atomfx://pair/<PAIR>`.
