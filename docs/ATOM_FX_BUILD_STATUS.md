...
| **Header gear + Insights icon (§3.1)** | ✅ | Persistent gear strip above the `HorizontalPager` on all 3 tabs → opens `SettingsScreen` full-screen. Regime name moved onto the wheel's own hub (no longer in the header); recommendation now lives in the status-strip glyph row, not the header either. **Correction 2026-09-10**: the hub's `strengthWord`/`flowLine` fields are computed (`WheelMapper.mapNucleus`) but never actually rendered — `WheelCanvas.drawHub` only draws the regime name. Flagged as an open product question in `ATOM_FX_DESIGN.md` §20, not fixed here |
