...
| **Whole Market Indicators card (Insights tab, Design §19.4)** | ✅ | `ui/insights/MarketIndicatorsCard.kt` + `RotationChart.kt`/`PulseChart.kt`/`ThrustChart.kt`/`PercentBBoardChart.kt` — four concept indicators (renamed 2026-09-10: Relative Rotation/Confidence Index/Breadth Thrust/%B), tried live so Pieter can decide which (if any) earns a permanent home. Same-day restyle: a `SheetTabs` 4-way row → a single "+ <current>" picker button fanning out a tap-to-select list (Item Library #3's reflow mechanic), with a spring-based "bump and settle" press animation on the button itself. An earlier on-device rename panel (also Item #3) was built then removed once real names were chosen. Verified on-device in both themes |
| **%B, per-pair (long-press → ChartSheet, Design §19.4a)** | ✅ | `ui/sheets/PercentBChart.kt` + rewritten `ChartSheet.kt`. Started the same day as a 4th Pair-sheet `BreakdownSection`, then moved the same day (Pieter's follow-up ask) onto the wheel's long-press sheet, replacing that sheet's old D1/H4/H1 price `LineChart` (a duplicate of the header's own always-visible `Spark3Row`) and its now-purposeless TF switcher — %B is D1-only. `ChartSheet` now shows a `%B <value>` + band-touch-state header row above the same shared `ui/chart/PercentBOscillator.kt` drawing the Board %B tab uses (one chart primitive, two data sources). Verified on-device both ways: opens correctly on long-press, and the Breakdown tab no longer shows it. **Same-day follow-up (schema v5):** Pieter caught the value itself not matching a live LiteFinance USDJPY %B read — traced to `bb_d1` reading the frozen aggregator's UTC-midnight D1, while retail platforms close D1 at 17:00 New York; `bb_touch.py` now builds its own D1 via `_d1_ny_close()` (independent re-bucketing of `raw_ohlcv`, same Rule #1-safe pattern the dates fix used, never touches the frozen aggregator). Not yet re-verified against a live chart post-fix — needs a fresh scan first (the fix landed between scans) |
| **Currency %B (`percent_b_currency`, schema v6)** | ✅ | `bb_touch.py`'s new `compute_currency_percent_b()` — Pieter noticed Board %B mixes every pair's raw %B with no regard for base/quote (EUR/USD falling = USD strong, USD/CAD falling = USD weak, averaged together unfixed). Same base/quote correction `csm.py`'s own `compute_csm_d1` already does, mirrored around 100 instead of negated (%B is a 0-100 position, not a signed return). Uses `csm.py`'s `STRENGTH_PAIRS` (18 pairs) for full currency coverage, independently computed from `raw_ohlcv` rather than reading `pairs.<PAIR>.bb_d1`. 65/65 tests pass (2 new, incl. a hand-verified base/quote sign-correctness check), Rule #1 holds. **Two UI surfaces, both same-day follow-ups**: (1) `ui/insights/CurrencyPercentBChart.kt` — a new "Currency %B" item in the Whole Market Indicators picker (renamed sibling "%B" → "Board %B" for clarity), own 8-currency pill selector, defaults USD; (2) `ChartSheet.kt`'s `CurrencyPercentBCard` — the pair's own base+quote currencies, each a small card below the pair's own %B, so a currency-level fragility read can be checked against whether the pair's other leg agrees (Pieter's own workflow, talked through first, then built). Both verified on-device |
