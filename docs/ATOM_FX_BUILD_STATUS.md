...
| **Market Indicators card (Insights tab, Design §19.4)** | ✅ | `ui/insights/MarketIndicatorsCard.kt` + `RotationChart.kt`/`PulseChart.kt`/`ThrustChart.kt`/`PercentBBoardChart.kt` — four concept indicators behind a 4-tab switcher, tried live so Pieter can decide which (if any) earns a permanent home. Verified on-device in both themes via a crafted local cache |
