...
| **Market Indicators card (Insights tab, Design §19.4)** | ✅ | `ui/insights/MarketIndicatorsCard.kt` + `RotationChart.kt`/`PulseChart.kt`/`ThrustChart.kt` (all new) — three concept indicators behind a 3-tab switcher, tried live so Pieter can decide which (if any) earns a permanent home. Verified on-device in both themes via a crafted local cache (real `signals.json` doesn't have the new keys until this ships and a real scan runs) |
