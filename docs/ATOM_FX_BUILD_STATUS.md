...
| **%B, per-pair (Pair sheet → Breakdown, Design §19.4a)** | ✅ | `ui/sheets/PercentBChart.kt` (new) — a 4th `BreakdownSection` alongside ALIGNMENT/MOMENTUM/STRUCTURE. Shares `ui/chart/PercentBOscillator.kt`'s drawing with the Board %B tab above (one chart primitive, two data sources). Verified on-device — renders correctly under STRUCTURE, doesn't visibly disturb the Overview/Breakdown height match |
