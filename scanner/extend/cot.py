"""
ATOM FX — CFTC COT downloader and parser  (EXTEND, Signals Roadmap §4)

Ported near-verbatim from `fx_technical/scanner/cot.py` (2026-09-04) — this file has no
ATOM-specific adaptation; it's self-contained (stdlib `urllib`/`zipfile`/`csv` only) and
transfers directly. Uses the CFTC Traders in Financial Futures (TFF) report:
fut_fin_txt_YYYY.zip — the correct file for currency futures (EUR, GBP, JPY, CHF, AUD,
CAD, NZD). No USD contract exists in TFF (standard COT convention); `conviction.py`
handles that absence per-currency, not here.
...
Lev_Money is the speculative signal (COT inputs 1 & 2); Asset_Mgr vs Lev_Money
divergence is the disaggregated signal (COT input 3). Both come from the same
fut_fin_txt file. Weekly cadence only (CFTC publishes Fridays, covering the prior
Tuesday) — this module is called from `scanner/scan_cot.py`, never from `scan_h1.py`.
"""
...
HEADERS = {"User-Agent": "Mozilla/5.0 (compatible; AtomFX/1.0)", "Accept": "*/*"}
